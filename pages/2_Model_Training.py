...
def main() -> None:
    ...
    if metadata:
        ...
        variants = metadata.get("model_variants", {})
        if variants:
            st.markdown("### Prediction Mode Options")
            v1, v2 = st.columns(2)
            without_marks = variants.get("without_marks", {})
            with_marks = variants.get("with_marks", {})
            with v1:
                render_metric_card(
                    "Without Marks",
                    f"{without_marks.get('best_accuracy', 0):.2%}",
                )
                st.caption("Uses individual profile parameters only.")
            with v2:
                render_metric_card(
                    "With Marks",
                    f"{with_marks.get('best_accuracy', 0):.2%}",
                )
                st.caption("Uses profile parameters plus score columns.")
